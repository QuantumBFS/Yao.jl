...
@testset "eigen basis" begin
    for op in [X, Y, H, Z, put(4, 2=>X), kron(X, Y, Z),
            repeat(3, H, 2:3), cache(Rx(0.5)), Daggered(Rx(0.5)),
            rot(SWAP, 0.5), time_evolve(kron(X,X), 0.5),
            3*put(3,2=>X)]
        @show op
...
@testset "better operator measure" begin
    Random.seed!(21)
    nbit=3
    for op in [put(nbit, 2=>X), control(nbit, 2, 1=>X), kron(X,X,X), put(nbit, (3,1,2)=>kron(X,Y,Z)),
        ...
        reg2 = copy(reg)
        @test isapprox(dropdims(sum(measure(op, reg2; nshots=100000), dims=1), dims=1)/100000, expect(op, reg), rtol=0.1)
        @test reg ≈ reg2
    end
end
...
@testset "measure op, specify locs" begin
    ...
    for op in [put(nbit, 2=>X), control(nbit, 2, 1=>X), kron(X,Y,Z), put(nbit, (3,1,2)=>kron(X,Y,Z)),
        repeat(nbit, X, 2:3), +(put(nbit, 2=>X), put(nbit, 1=>Rx(π))),
        2.8 * put(nbit, 1=>X), chain(nbit, put(nbit, 3=>X), put(nbit, 1=>Z)),
        cache(put(nbit, 2=>X)), Daggered(put(nbit, 2=>Rx(0.8)))
        ]
        ...
        reg2 = copy(reg)
        @show op
        @test isapprox(sum(measure(op, reg2, locs; nshots=100000))/100000, expect(put(Nbit,locs=>op), reg), rtol=0.2)
        @test reg ≈ reg2
    end
end
...
@testset "better operator measure!" begin
    Random.seed!(9)
    nbit=3
    for op in [put(nbit, 2=>X), control(nbit, 2, 1=>X), kron(X,X,X), put(nbit, (3,1,2)=>kron(X,X,X)),
        repeat(nbit, X, 2:3), +(put(nbit, 2=>X), put(nbit, 1=>im*Rx(π))),
        2.8 * put(nbit, 1=>X), chain(nbit, put(nbit, 3=>X), put(nbit, 1=>Z)),
        cache(put(nbit, 2=>X)), Daggered(put(nbit, 2=>im*Rx(π)))
        ]
        @show op
        reg = rand_state(nbit)
        res = measure!(op, reg)
