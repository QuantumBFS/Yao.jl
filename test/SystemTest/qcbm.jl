...
@testset "qcbm" begin
    ...
    for i in 1:num_layer
        ...
        append!(circuit, rotation_block(num_bit, zxz_mask=[i!=0, true, i!=num_layer]) |> cache(cache_level=1, recursive=true))
    end

    kernel = hilbert_rbf_kernel(2, [0.3, 3.0], false)

    for info in optimize(loss_function, args=(circuit, kernel))
        println(info)
        println("loss is ", loss_function(kernel, prob, ptrain))
    end
end

function run_circuit(params::Vector{Float64}, circuit::Sequence)
    psi = Register("0"^num_qubit)

    local final_state
    for info in psi >> (circuit |> scatter_params(params) |> cache_signal(3))   # here, cache signal uses default cache threshhold and can be avoided
...
QCBM loss function.
    we can use
        samples = measure(final_state, num_sample=20000)
    to get samples, here, we simply use the exact wave function.
"""
loss_function(params::Vector{Float64}, circuit::Sequence, kernel::Kernel, ptrain::Vector{Float64}) = run_circuit(params, circuit) |> psi2prob |> mmd_loss(kernel, ptrain)

"""
QCBM gradient function.
"""
function mmd_gradient(params, ptrain)
    prob = run_circuit(params, circuit) |> psi2prob

    function get1(i)
        params_ = copy(params)
        # pi/2 phase
        params_[i] += pi/2.
        prob_pos = run_circuit(params_, circuit) |> psi2prob
        # -pi/2 phase
        params_[i] -= pi
        prob_neg = run_circuit(params_, circuit) |> psi2prob
