...
@testset "rotter, collect_rotblocks, num_gradient, opgrad" begin
    ...
    @test mean(opgrad(()->expect(obs, apply!(zero_state(4), c))|>real, rots) .|> abs) > 3e-3
    @test isapprox(opgrad(()->expect(obs, apply!(zero_state(4), c))|>real, rots),  num_gradient(()->expect(obs, apply!(zero_state(4),c))|>real, rots), atol=1e-3)
