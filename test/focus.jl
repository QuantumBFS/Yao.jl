...
import BitBasis: IntIterator

function naive_focus!(reg::ArrayReg{B}, bits::IntIterator) where B
    nbit = nqubits(reg)
...
function naive_relax!(reg::ArrayReg{B}, bits::IntIterator) where B
    nbit = nqubits(reg)
