...
@testset "map address" begin
    ...
    # control, kron, rot
    c3 = map_address(chain(5, control(5, 2, 1=>Rx(0.3)), kron(5, 2=>Y, 3=>X)), AddressInfo(10, [2,1,4,6,3]))
    @test c3 == chain(10, control(10, 1, 2=>Rx(0.3)), kron(10, 1=>Y, 4=>X))

    # repeat, measure
    c4 = map_address(chain(5, repeat(5, Y, (2,3)), Measure(5), Measure(5; locs=(3,2,1))), AddressInfo(10, [2,1,4,6,3]))
    @test c4 == chain(10, repeat(10, Y, (1,4)), Measure(10; locs=(2,1,4,6,3)), Measure(10; locs=(4,1,2)))

    # sum, cache, scale
    c5 = map_address(2*put(5, 2=>X) + Daggered(Scale(Val(2), put(5, 3=>X) |> cache)), AddressInfo(10, [2,1,4,6,3]))
...
@testset "to basic types" begin
    ...
    @test to_basictypes(Measure(5; operator=repeat(3,X,1:3), locs=(3,2,1))) == Measure(5; operator=repeat(3,X,1:3),locs=(3,2,1))
    ...
    # tobasic_recursive
    sub = chain(5, Daggered(put(5,4=>Rx(0.5))*2), put(5,3=>ConstGate.P0+ConstGate.P1),
            kron(5, 3=>X, 4=>Y), concentrate(5, kron(X,Z), (3,2)),
            Measure(5, operator=X, locs=1))
    c = chain(10, repeat(10, H, 1:10),
            concentrate(10, sub, 6:10))

    sub2 = chain(10, Daggered(put(10,9=>Rx(0.5))*2), put(10,8=>ConstGate.P0+ConstGate.P1),
            chain(put(10, 8=>X), put(10, 9=>Y)), chain(put(10,8=>X),put(10,7=>Z)),
    ...
    c = chain(6, [put(6, 3=>X), kron(6, 2=>X, 4=>X), chain(6, [concentrate(6, control(2, 1, 2=>Y), (6,1))])])
    @test zero_state(6) |> c ≈ zero_state(6) |> simplify(c, rules=[to_basictypes])
end

@testset "replace block" begin
    @test eliminate_nested(chain(7,chain(7,control(7, 1,2=>X)), put(7, 4=>X))) == chain(7, [control(7, 1,2=>X), put(7, 4=>X)])
    @test replace_block(X=>Y, chain(put(2,2=>X), put(2,1=>Z), kron(X, Y))) == chain(put(2,2=>Y), put(2,1=>Z), kron(Y, Y))
end
