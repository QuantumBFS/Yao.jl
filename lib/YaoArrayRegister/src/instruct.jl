...
function YaoBase.instruct!(
    r::ArrayReg, op, locs::Tuple, control_locs::Tuple, control_bits::Tuple
)
    return instruct!(r.state, op, locs, control_locs, control_bits)
end

function YaoBase.instruct!(
    r::ArrayReg{1}, op, locs::Tuple, control_locs::Tuple, control_bits::Tuple
)
    return instruct!(vec(r.state), op, locs, control_locs, control_bits)
end

function YaoBase.instruct!(r::ArrayReg, op, locs::Tuple)
    return instruct!(r.state, op, locs)
end

function YaoBase.instruct!(r::ArrayReg{1}, op, locs::Tuple)
    return instruct!(vec(r.state), op, locs)
end

function YaoBase.instruct!(
    r::ArrayReg, op, locs::Tuple, control_locs::Tuple, control_bits::Tuple, theta::Number
)
    return instruct!(r.state, op, locs, control_locs, control_bits, theta)
end

function YaoBase.instruct!(r::ArrayReg, op, locs::Tuple, theta::Number)
    return instruct!(r.state, op, locs, theta)
end
...
"""
const SPECIALIZATION_LIST = Symbol[
    :X, :Y, :Z, :S, :T, :Sdag, :Tdag, :H, :SWAP, :PSWAP, :CPHASE
]
...
function YaoBase.instruct!(
    ...
    return instruct!(
        state, copyto!(similar(operator, T1), operator), locs, control_locs, control_bits
    )
end

function _prepare_instruct(
    state, U, locs::NTuple{M}, control_locs, control_bits::NTuple{C}
) where {M,C}
...
function _instruct!(
    state::AbstractVecOrMat{T}, U::AbstractMatrix{T}, locs_raw::SVector, ic::IterControl
) where {T}
    @threads for j in 1:length(ic)
        @inbounds i = ic[j]
...
function _instruct!(
    state::AbstractVecOrMat{T}, U::SDSparseMatrixCSC{T}, locs_raw::SVector, ic::IterControl
) where {T}
    work = if ndims(state) == 1
        similar(state, length(locs_raw))
    else
...
function YaoBase.instruct!(
    state::AbstractVecOrMat, U::IMatrix, locs::NTuple{N,Int}
) where {N}
    return state
end
...
function YaoBase.instruct!(
    state::AbstractVecOrMat{T}, U1::AbstractMatrix{T}, (loc,)::Tuple{Int}
) where {T}
    a, c, b, d = U1
...
function YaoBase.instruct!(
    state::AbstractVecOrMat{T}, U1::SDPermMatrix{T}, (loc,)::Tuple{Int}
) where {T}
...
function YaoBase.instruct!(
    state::AbstractVecOrMat{T}, U1::SDDiagonal{T}, (loc,)::Tuple{Int}
) where {T}
    mask = bmask(loc)
    a, d = U1.diag
    step = 1 << (loc - 1)
...
# paulis
function YaoBase.instruct!(
    state::AbstractVecOrMat, ::Val{:X}, locs::NTuple{N,Int}
) where {N}
    mask = bmask(locs)
...
function YaoBase.instruct!(
    state::AbstractVecOrMat, ::Val{:H}, locs::NTuple{N,Int}
) where {N}
    return instruct!(state, YaoBase.Const.H, locs)
end

function YaoBase.instruct!(
    state::AbstractVecOrMat{T}, ::Val{:Y}, locs::NTuple{N,Int}
) where {T,N}
    mask = bmask(Int, locs)
...
function YaoBase.instruct!(
    state::AbstractVecOrMat{T}, ::Val{:Z}, locs::NTuple{N,Int}
) where {T,N}
    mask = bmask(Int, locs)
    @threads for b in basis(Int, state)
...
for (G, FACTOR) in zip(
    [:S, :T, :Sdag, :Tdag], [:(im), :($(exp(im * π / 4))), :(-im), :($(exp(-im * π / 4)))]
)
    @eval function YaoBase.instruct!(
        state::AbstractVecOrMat{T}, ::Val{$(QuoteNode(G))}, locs::NTuple{N,Int}
    ) where {T,N}
        mask = bmask(Int, locs)
        @threads for b in basis(Int, state)
...
for (G, FACTOR) in zip(
    ...
    @eval function YaoBase.instruct!(
        state::AbstractVecOrMat{T}, ::Val{$(QuoteNode(G))}, (loc,)::Tuple{Int}
    ) where {T}
        mask = bmask(loc)
        step = 1 << (loc - 1)
        step_2 = 1 << loc
...
function rot_mat(::Type{T}, ::Val{:Rx}, theta::Number) where {T}
    return T[cos(theta / 2) -im*sin(theta / 2); -im*sin(theta / 2) cos(theta / 2)]
end
function rot_mat(::Type{T}, ::Val{:Ry}, theta::Number) where {T}
    return T[cos(theta / 2) -sin(theta / 2); sin(theta / 2) cos(theta / 2)]
end
function rot_mat(::Type{T}, ::Val{:Rz}, theta::Number) where {T}
    return Diagonal(T[exp(-im * theta / 2), exp(im * theta / 2)])
end
function rot_mat(::Type{T}, ::Val{:CPHASE}, theta::Number) where {T}
    return Diagonal(T[1, 1, 1, exp(im * theta)])
end
...
@inline function YaoBase.instruct!(
    state::AbstractVecOrMat{T}, ::Val{:Rx}, (loc,)::Tuple{Int}, theta::Number
) where {T,N}
    b, a = sincos(theta / 2)
...
function YaoBase.instruct!(
    state::AbstractVecOrMat{T}, ::Val{:Ry}, (loc,)::Tuple{Int}, theta::Number
) where {T,N}
    b, a = sincos(theta / 2)
...
function YaoBase.instruct!(
    state::AbstractVecOrMat{T}, ::Val{:Rz}, (loc,)::Tuple{Int}, theta::Number
) where {T,N}
    a = exp(-im * theta / 2)
...
function YaoBase.instruct!(
    state::AbstractVecOrMat{T}, ::Val{:CPHASE}, locs::NTuple{N,Int}, theta::Number
) where {T,N}
...
# forward single gates
function YaoBase.instruct!(
    state::AbstractVecOrMat{T}, g::Val, locs::Union{Int,NTuple{N1,Int}}, theta::Number
) where {T,N1}
    return instruct!(state, g, locs, (), (), theta)
end
...
for G in [:X, :Y, :Z, :S, :T, :Sdag, :Tdag]
    ...
    @eval function YaoBase.instruct!(
        ...
        return instruct!(state, g, locs, control_locs..., control_bits...)
    end
...
function YaoBase.instruct!(
    state::AbstractVecOrMat{T}, ::Val{:X}, loc::Int, control_locs::Int, control_bits::Int
) where {T}
    mask2 = bmask(loc)
...
function YaoBase.instruct!(
    state::AbstractVecOrMat{T}, ::Val{:Y}, loc::Int, control_locs::Int, control_bits::Int
) where {T}
    mask2 = bmask(loc)
    mask = bmask(control_locs, loc)
    step = 1 << (control_locs - 1)
...
# 2-qubit gates
function YaoBase.instruct!(
    state::AbstractVecOrMat{T}, ::Val{:SWAP}, locs::Tuple{Int,Int}
) where {T}
    mask1 = bmask(locs[1])
    mask2 = bmask(locs[2])
    mask12 = mask1 | mask2
...
function YaoBase.instruct!(
    state::AbstractVecOrMat{T}, ::Val{:PSWAP}, locs::Tuple{Int,Int}, theta::Number
) where {T}
    mask1 = bmask(locs[1])
    mask2 = bmask(locs[2])
    mask12 = mask1 | mask2
...
function YaoBase.instruct!(
    ...
    @threads for b in itercontrol(
        log2i(size(state, 1)), collect(Int, control_locs), collect(Int, control_bits)
    )
        if b & mask1 == 0
            i = b + 1
            i_ = b ⊻ mask12 + 1
