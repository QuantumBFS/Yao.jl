...
chain(blocks::Union{AbstractBlock{N},Function}...) where {N} =
...
chsubblocks(pb::ChainBlock{N}, blocks::Vector{<:AbstractBlock}) where {N} =
    length(blocks) == 0 ? ChainBlock{N}([]) : ChainBlock(blocks)
