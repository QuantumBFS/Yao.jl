...
for n in 5:5:25
    ...
    SUITE["composite"]["kron(sparse_const)"] = bench(
        kron(n, k => rand([X, Y, Z, H]) for k in randperm(n)[1:(n ÷ 5)])
    )
...
for n in 5:5:25
    SUITE["primitive"]["TimeEvolution(heisenberg(n), 0.2)"] = bench(
        TimeEvolution(heisenberg(n), 0.2)
    )
end
