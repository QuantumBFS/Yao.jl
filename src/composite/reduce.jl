...
Base.setindex!(c::Add{N}, val::AbstractBlock{N}, index::Integer) where N = (setindex!(c.list, val, index); c)
Base.insert!(c::Add{N}, index::Integer, val::AbstractBlock{N}) where N = (insert!(c.list, index, val); c)
