...
mat(c::ControlBlock{N, BT, C}) where {N, BT, C} = cunmat(N, c.ctrl_qubits, c.vals, mat(c.content), c.addrs)

function apply!(r::ArrayReg, c::ControlBlock)
    instruct!(matvec(r.state), mat(c.content), c.addrs, c.ctrl_qubits, c.vals)
    return r
end

PreserveStyle(::ControlBlock) = PreserveAll()

occupied_locations(c::ControlBlock) = (c.ctrl_qubits..., map(x->c.addrs[x], occupied_locations(c.content))...)
...
function Base.:(==)(lhs::ControlBlock{N, BT, C, M, T}, rhs::ControlBlock{N, BT, C, M, T}) where {BT, N, C, M, T}
    return (lhs.ctrl_qubits == rhs.ctrl_qubits) && (lhs.content == rhs.content) && (lhs.addrs == rhs.addrs)
end

Base.adjoint(blk::ControlBlock{N}) where N = ControlBlock{N}(blk.ctrl_qubits, blk.vals, adjoint(blk.content), blk.addrs)
...
function Base.copy(ctrl::ControlBlock{N, BT, C, M, T}) where {BT, N, C, M, T}
    return ControlBlock{N, BT, C, M, T}(ctrl.ctrl_qubits, ctrl.vals, ctrl.content, ctrl.addrs)
end
