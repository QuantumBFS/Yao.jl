...
YaoAPI.subblocks(seq::Sequence) = filter(x->x isa AbstractBlock, seq.blocks)
YaoAPI.chsubblocks(pb::Sequence, blocks::Vector) = Sequence(blocks)

function YaoAPI.apply!(reg::ArrayReg, seq::Sequence)
    for x in seq.blocks
        reg |> x
    end
    reg
