...
function Matrix{T}(M::PermMatrix) where T
    n = size(M, 1)
    Mf = zeros(T, n, n)
    @inbounds for i=1:n
...
Matrix(M::PermMatrix{T}) where {T} = Matrix{T}(M)
Array(M::PermMatrix) = Matrix(M)
full(M::PermMatrix) = Matrix(M)
pmrand(T::Type, n::Int) = PermMatrix(randperm(n), randn(T, n))
pmrand(n::Int) = PermMatrix(randperm(n), randn(n))

function sparse(M::PermMatrix{T}) where {T}
    n = size(M, 1)
