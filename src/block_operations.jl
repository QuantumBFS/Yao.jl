...
"""
function Base.collect(circuit::AbstractBlock, ::Type{BT}) where BT<:AbstractBlock
...
function Base.iterate(it::BlockTreeIterator{:BFS}, st = (q = Queue(AbstractBlock); enqueue!(q, itr.root)) )
    if isempty(st)
        nothing
    else
        node = dequeue!(st)
...
# Depth First Search
function Base.iterate(it::BlockTreeIterator{:DFS}, st = AbstractBlock[it.root])
    if isempty(st)
        nothing
    else
        node = pop!(st)
