...
for METHOD in (:ishermitian, :isreflexive, :isunitary)
    @eval YaoBase.$METHOD(c::AbstractContainer) = $METHOD(c, istraitkeeper(c))
    @eval YaoBase.$METHOD(c::AbstractContainer, ::Val{true}) = $METHOD(block(c))
    @eval YaoBase.$METHOD(c::AbstractContainer, ::Val{false}) = $METHOD(mat(c))
end
