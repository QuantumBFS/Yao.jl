...
Base.copy(m::Roller) = typeof(m)(m.blocks)

subblocks(m::Roller) = m.blocks
addrs(m::Roller) = cumsum([[1]; [nqubits(b) for b in m.blocks[1:end-1]]])
YaoBase.isunitary(m::Roller) = all(isunitary, m.blocks) || isunitary(mat(m))
YaoBase.ishermitian(m::Roller) = all(ishermitian, m.blocks) || ishermitian(mat(m))
YaoBase.isreflexive(m::Roller) = all(isreflexive, m.blocks) || isreflexive(mat(m))
chsubblocks(pb::Roller, blocks) where N = Roller((blocks...,))

mat(m::Roller) = mapreduce(blk->mat(blk), kron, m.blocks[end:-1:1])
...
Base.:(==)(lhs::Roller{N, T, BT}, rhs::Roller{N, T, BT}) where {N, T, BT} = lhs.blocks == rhs.blocks

function Base.hash(R::Roller, h::UInt)
