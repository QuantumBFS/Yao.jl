...
YaoBase.nactive(c::Concentrator) = length(c.usedbits)
...
YaoBase.iscommute(x::Concentrator{N}, y::Concentrator{N}) where N = x.usedbits == y.usedbits ? iscommute(x.block, y.block) : _default_iscommute(x, y)

apply!(reg::AbstractRegister, c::Concentrator) = relax!(apply!(focus!(reg, usedbits(c)), c.block), usedbits(c), nbit=nqubits(c))
Base.adjoint(blk::Concentrator{N}) where N = Concentrator{N}(adjoint(blk.block), blk.usedbits)
...
Base.:(==)(a::Concentrator{N, T, BT}, b::Concentrator{N, T, BT}) where {N, T, BT} = a.block == b.block && a.usedbits == b.usedbits
